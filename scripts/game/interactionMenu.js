...
function drawIMenu() {
  if ( Player.drawInv == true ) {
	...
	ctx.fillStyle = "black";
	if ( i == 0 ) {
  	   ctx.fillText("Weapons",width*.13,height*((.90-.075*options.length+0.08)+0.08*(i/2 + j)));	    
    	   j++;
    	}
	if ( options[i] == "potion" ) {
	   j++;
	   ctx.fillText("Consumables",width*.13,height*((.90-.075*options.length+0.08)+0.08*(i/2 + j)));	
	   j++;
	}
	if ( i+1 == options.length )
       ...
       if ( currentOption == i )
        ...
        ctx.fillText(options[i],width*.13,height*((.90-.075*options.length+0.08)+0.08*(i/2 + j)));
    }
  }
  else {
    ctx.fillStyle = "#FFFFFF";
